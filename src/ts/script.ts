...
export function sliderInit () {
  let runnerNumber: number = configObj.runner_number;
  ...
  let scaleArr = scaleArrs [0] as number[];        
  let iteration = scaleArrs [1] as number;
  let iterationsArr = scaleArrs [2] as number[];
...
export function setStructure (
  ...
  elements.forEach((elem) => {
    ...
    let rangerDiv = document.querySelectorAll('.ranger')[i] as HTMLElement;
    ...
    let confInputMin = document.querySelectorAll('.zdslider-panel__min')[i] as HTMLInputElement;
    ...
    let confInputMax = document.querySelectorAll('.zdslider-panel__max')[i] as HTMLInputElement;
    ...
    let confInputStep = document.querySelectorAll('.zdslider-panel__step')[i] as HTMLInputElement;
...
/* Первоначальное размещение слайдера */
export function sliderPositioning (runners: number, orientation: string) {   
  let elements = document.querySelectorAll('.zdslider');
  let i = 0;
  ...
  elements.forEach((elem) => {
    ...
    let button1 = document.querySelectorAll('[data-type="btn-first"]')[i] as HTMLElement; 
    ...
    if (runners === 2) {
      if (orientation === 'horizontal') {
        ...
        let button2 = document.querySelectorAll('[data-type="btn-second"]')[i] as HTMLElement;
